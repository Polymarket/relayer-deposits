...
describe("Unit tests", function () {
    describe("DepositRouter", function () {
        ...
        it("admin is the admin", async () => {
            ...
            expect(
                await router.hasRole(ADMIN_ROLE, admin.address)
            ).to.equal(true);
        });
        ...
        it("admin has the relayer role", async () => {
            ...
            expect(
                await router.hasRole(relayerRole, admin.address)
            ).to.equal(true);
        });
        ...
        it("reverts when address other than admin tries to claim fees", async () => {
            ...
            await expect(
                router.connect(randomSigner).claimFees(randomSigner.address, 100)
            ).to.be.revertedWith(
        ...
        it("reverts when address other than a relayer tries to deposit", async () => {
            ...
            await expect(router.connect(randomSigner).deposit(
                admin.address,
                admin.address,
                depositAmount,
                fee,
